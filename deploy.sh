...
if [[ "$CLUSTER_TYPE" == "minikube" ]]; then
  ...
  kind load docker-image platform-frontend:v2
  kind load docker-image auth-server:latest
else
  echo "[!] No cluster type specified; skipping image load."
  echo "    Run manually: minikube image load shop-frontend:latest && minikube image load platform-frontend:v2 && minikube image load auth-server:latest"
  echo "               or: kind load docker-image shop-frontend:latest && kind load docker-image platform-frontend:v2 && kind load docker-image auth-server:latest"
fi
